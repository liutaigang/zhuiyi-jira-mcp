...
function fieldText(fields: AnyRecord, fieldId: string | undefined): string {
...
export function normalizeIssue(
  ...
  const acceptanceCriteria = fieldText(
    fields,
    options.requirementFields.acceptanceCriteria
  );

  const customFields = Object.fromEntries(
    Object.entries(options.requirementFields).map(([name, fieldId]) => [
      name,
      fields[fieldId]
    ])
  );
  ...
  const comments = options.includeComments
    ? (record(fields.comment).comments as unknown[] | undefined)?.map((comment) => {
